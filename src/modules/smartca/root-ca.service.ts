...
export class RootCAService implements OnModuleInit {
  ...
  onModuleInit() {
    try {
      ...
      this.logger.warn('Root CA not loaded at startup: ' + String(err?.message || err));
    }
  }

  loadRootCA() {
    // Read config via ConfigService (use rootCA config registered in AppModule)
    const cfg = this.configService.get('rootCA') as Record<string, unknown> | undefined;
    const systemKey = (cfg?.['systemEncKey'] as string) || process.env.SYSTEM_ENC_KEY;
    if (!systemKey) throw new Error('SYSTEM_ENC_KEY is required to decrypt Root CA key');

    const encB64 = (cfg?.['rootCaKeyEncB64'] as string) || process.env.ROOT_CA_KEY_ENC_B64 || process.env.ROOT_CA_KEY_ENC_BASE64;
    const keyB64 = (cfg?.['rootCaKeyB64'] as string) || process.env.ROOT_CA_KEY_B64 || process.env.ROOT_CA_KEY_BASE64;
    const certB64 = (cfg?.['rootCaCertB64'] as string) || process.env.ROOT_CA_CERT_B64 || process.env.ROOT_CA_CERT_BASE64;
    ...
    if (encB64) {
      ...
      const encPath = (cfg?.['rootCaEncPath'] as string) || process.env.ROOT_CA_ENC_PATH || path.join(__dirname, '../../../src/certs/root_ca_key.pem.enc');
      if (!fs.existsSync(encPath)) throw new Error(`Encrypted Root CA key not found: ${encPath}`);
    ...
    if (certB64) {
      ...
      const certPath = (cfg?.['rootCaCertPath'] as string) || process.env.ROOT_CA_CERT_PATH || path.join(__dirname, '../../../src/certs/root_ca_cert.pem');
      if (!fs.existsSync(certPath)) throw new Error(`Root CA cert not found: ${certPath}`);
  ...
  getRootPrivateKeyPem(): string {
    if (!this.rootPrivateKeyPem) throw new Error('Root CA private key not loaded');
  ...
  signCertificate(opts: {
    ...
    const serial = opts.serialNumber ?? Math.floor(Math.random() * Date.now()).toString(16);
    ...
    const attrs = [] as any[];
    if (opts.subject?.CN) attrs.push({ name: 'commonName', value: opts.subject.CN });
    if (opts.subject?.O) attrs.push({ name: 'organizationName', value: opts.subject.O });
    if (opts.subject?.C) attrs.push({ name: 'countryName', value: opts.subject.C });
    if (opts.subject?.emailAddress) attrs.push({ name: 'emailAddress', value: opts.subject.emailAddress });
