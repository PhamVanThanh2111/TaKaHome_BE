...
export class ContractManagementCron {
   ...
   */
  private async sendContractExpiryReminder(contract: Contract, daysToExpiry: number): Promise<void> {
    try {
      ...
      this.logger.log(`📨 Sent ${daysToExpiry}-day contract expiry reminder for contract ${contract.id}`);

    } catch (error) {
      this.logger.error(`❌ Failed to send contract expiry reminder for contract ${contract.id}:`, error);
    }
  }
  ...
  async scheduleMaintenanceReminders(): Promise<void> {
    try {
      ...
      this.logger.log(`🔧 Sent maintenance reminders for ${activeContracts.length} active contracts`);
    } catch (error) {
